...
DATA_DIR = Path(os.getenv("RADHARNESS_DATA_DIR", BASE_DIR / "data"))
DICOM_DIR = Path(os.getenv("RADHARNESS_DICOM_DIR", DATA_DIR / "dicom"))
KB_DIR = Path(os.getenv("RADHARNESS_KB_DIR", DATA_DIR / "kb"))
FRAMES_DIR = Path(os.getenv("RADHARNESS_FRAMES_DIR", DATA_DIR / "frames"))
DB_PATH = Path(os.getenv("RADHARNESS_DB_PATH", DATA_DIR / "radharness.db"))
FRONTEND_DIR = Path(os.getenv("RADHARNESS_FRONTEND_DIR", BASE_DIR / "frontend"))
...
LLM_RUNTIME = os.getenv("RADHARNESS_LLM_RUNTIME", "ollama").lower()

OLLAMA_BASE_URL = os.getenv("RADHARNESS_OLLAMA_BASE_URL", "http://localhost:11434/v1")
FOUNDRY_BASE_URL = os.getenv("RADHARNESS_FOUNDRY_BASE_URL", "http://localhost:5273/v1")

# Model used for reasoning-heavy work (report drafting, triage, agent).
CHAT_MODEL = os.getenv("RADHARNESS_CHAT_MODEL", "gpt-oss:20b")
# Small fast model for lightweight classification / short tasks.
FAST_MODEL = os.getenv("RADHARNESS_FAST_MODEL", "qwen2.5-coder:1.5b")
...
EMBED_MODEL = os.getenv("RADHARNESS_EMBED_MODEL", "nomic-embed-text")

# Generation defaults
DEFAULT_TEMPERATURE = float(os.getenv("RADHARNESS_TEMPERATURE", "0.2"))
REQUEST_TIMEOUT = float(os.getenv("RADHARNESS_TIMEOUT", "180"))

# ---------------------------------------------------------------------------
# Privacy / safety
# ---------------------------------------------------------------------------
# When true, the LLM client only permits loopback model endpoints.
OFFLINE_MODE = os.getenv("RADHARNESS_OFFLINE_MODE", "1") not in ("0", "false", "False")

# Server bind (loopback only by default so the app is not exposed on the LAN).
HOST = os.getenv("RADHARNESS_HOST", "127.0.0.1")
PORT = int(os.getenv("RADHARNESS_PORT", "8000"))

DISCLAIMER = (
    "RadHarness is a local research and productivity assistant, NOT a medical "
